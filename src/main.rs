...
fn run_apply(config: &config::structs::Config) -> Result<()> {
    ...
    // 3c. Write persistent NetworkManager MAC address config
    if let Err(e) = write_nm_mac_config(&config.wifi.wifi_mac_address) {
...
/// Path for the persistent NetworkManager MAC address config
const NM_MAC_CONF: &str = "/etc/NetworkManager/conf.d/99-hifi-wifi-mac.conf";
...
fn write_nm_mac_config(mac: &Option<String>) -> Result<()> {
    ...
    if let Some(mac_val) = mac {
        ...
        match File::create(conf_path) {
            ...
            Err(e) => {
                warn!("Could not create NM MAC config: {}", e);
            }
        }
    } else if conf_path.exists() {
        let _ = fs::remove_file(conf_path);
        info!("Removed NM MAC config: {}", NM_MAC_CONF);
