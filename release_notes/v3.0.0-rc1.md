# hifi-wifi v3.0.0-rc1 Release Notes

**Release Date:** February 5, 2026

## Overview

RC1 integrates performance optimizations from external contributor cat-in-a-box, eliminating WiFi scan latency spikes and adding intelligent CAKE RTT detection. This release focuses on reducing latency and improving Steam Deck OLED support.
...
### NAPI Busy Polling Regression (Caught Pre-Release)

**Issue:** NAPI tuning (`napi_defer_hard_irqs`, `gro_flush_timeout`) caused "Unbalanced enable for IRQ 68" kernel warnings and micro-stuttering during streaming on QCA2066 WiFi (Steam Deck OLED)
**Discovery:** Caught during extended testing by cat-in-a-box
**Fix:** Removed all NAPI tuning code before RC1 release
**Impact:** Prevented regression from reaching users; scan suppression still provides 95% latency reduction without NAPI

### Automatic Privilege Escalation (Issue #15)

**Issue:** Commands requiring root would fail with "sudo: hifi-wifi: command not found" when binary not in system PATH
**Report:** WanderingxLotus ("Cmds not recognized")
**Fix:** Added automatic sudo elevation using `std::env::current_exe()` to get full binary path
**Impact:** Users can now run `hifi-wifi off` or other privileged commands without explicit sudo prefix
...
- cat-in-a-box: Scan suppression, power-save CLI, CAKE RTT detection, OLED IRQ fix, NAPI regression discovery and fix
